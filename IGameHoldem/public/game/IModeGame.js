  ...
  constructor(socket, listButtons, listBgs, listImages, kSC, kTimer) {
    ...
    this.listLabels = [
      ...
      new IUILabel(
        ...
        163,
        227,
        this.iTotalBettingCoin.toString(),
        2
      ),
      new IUILabel(
        1100,
    ...
    this.listImagesCardFrame = new IUIImage(
      0,
      0,
      130,
      180,
    ...
    this.gameWinCeremony = new IUIImage(
      343,
      130,
      1241,
      560,
    ...
    this.gameAbstentionWin = new IUIImage(
      490,
      90,
      950,
      403,
    ...
    this.gamewinMadePanel = new IUIImage(
      243,
      121,
      1441,
      800,
    ...
    this.showdownbg = new IScaleEffect(
      0,
      0,
      0.7,
      0.35,
    ...
    this.showdown = new IScaleEffect(
      0,
      0,
      0.7,
      0.35,
  ...
  Render(ctx) {
    ...
    if (this.listTableCard.length > 0) {
      for (let i in this.listTableCard) {
        //this.listImagesCard[this.listTableCard[i]].RenderLocation(ctx, cTableCardLocations[i].x, cTableCardLocations[i].y);
        this.listImagesCard[this.listTableCard[i]].CardRenderTurn(
          ctx,
          // cTableCardLocations[i].x,
          // cTableCardLocations[i].y,
          this.kSC.GetLocation(ELocationIndex.TableCard1+parseInt(i)).x,
          this.kSC.GetLocation(ELocationIndex.TableCard1+parseInt(i)).y,
          this.kTimer,
          i,
          this.strDeckcode
        );
        // if ( this.listWinCards[i] == true && this.bRender == true )
        // {
        //     this.listImagesCardFrame.RenderLocation(ctx, cTableCardLocations[i].x, cTableCardLocations[i].y);
        // }
        if (this.listWinCards[i] == true && this.bAbstentionWin == false) {
          this.listImagesCardFrame.RenderLocation(
            ctx,
            // cTableCardLocations[i].x,
            // cTableCardLocations[i].y
            this.kSC.GetLocation(ELocationIndex.TableCard1+parseInt(i)).x,
      ...
      if (this.listTableCardTemp.length > 0) {
        if (this.bEnableDealingCard == true) {
          ...
          if (this.fDealingCardElapsedTime > 0.2) {
            ...
            //let iCard = this.listTableCardTemp[this.listTableCard.length];
            ...
            let dealer = new ICardDealer(
              cDealerLocation.x,
              cDealerLocation.y,
              // cTableCardLocations[this.iNumCards].x,
              // cTableCardLocations[this.iNumCards].y,
              this.kSC.GetLocation(ELocationIndex.TableCard1+this.iNumCards).x,
              this.kSC.GetLocation(ELocationIndex.TableCard1+this.iNumCards).y,
              this.kSC.m_fWidthRate,
              this.kSC.m_fHeightRate,
              this.kTimer,
              130,
              170,
              this.listTableCardTemp[0],
              this.strDeckcode
            );
