...
export let shouldAutoPass = (listHandCard, listTableCard) => {
    ...
    const hasSpadeTwoTriple = listTableCard.filter(card => card % 13 === 1).length === 3;
...
export let generateValidCombinations = (handCards, tableCards) => {
  ...
  const getAllCombinations = (handCards) => {
    ...
    const singles = GetSingleCards(handCards);
  ...
  let findLowestCard = (handCards) => {
    // 가장 낮은 카드를 찾는 함수를 작성하세요.
    // 예시: 카드 숫자와 무늬에 대한 정보가 필요하며, 카드 정렬 방식에 따라 달라집니다.
